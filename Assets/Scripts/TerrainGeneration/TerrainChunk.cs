...
public class TerrainChunk
{
		...
		Transform parent, Material material, HeightMapSettings _heightSettings, MeshSettings _meshSettings, Transform _viewer)
	{
		...
		{
			lodMeshes[i] = new LODMesh(detailLevels[i].lod, sampleCentre);
    ...
    public void Load()
	{
        ThreadedDataRequestor.RequestData(() => HeightMapGenerator.GetHeightMap(meshSettings.numVertsPerLine, meshSettings.numVertsPerLine, heightSettings, sampleCentre),
            OnHeightMapReceived);
    }
	...
	class LODMesh
	{
		...
		public void RequestMesh(HeightMap heightmap, MeshSettings settings)
		{
			...
			ThreadedDataRequestor.RequestData(() => MeshGenerator.GenerateTerrainMesh(heightmap, lod, settings), OnMeshDataReceived);
		}
